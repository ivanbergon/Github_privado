...
Plotting the `OILPRICE` time serie.

```{r}
plot(df$OILPRICE, type = "l")
```
...
### 8 - FORECASTING ONE DAY AHEAD

Now to forecast forward we need to refit the whole time series including the test set, since recent prices are important for our model.So what will we do is fit the whole time series data and forecast to the horizon we had established which was established at “60 days.”


```{r}
# check if model exists? If not, refit:
if(file.exists("gamlss_oil_model_full.rda")) {
    # load model
    load("gamlss_oil_model_full.rda")
 } else {
    # (re)fit the model
    model_df_full <- gamlss(formula = OILPRICE ~ ., family = BCPE, data = df, control = gamlss.control(n.cyc = 150)) 
 }

# save the model
save(model_df_full, file = "gamlss_oil_model_full.rda")
```


```{r}
pdf.plot(model_df_full, min = 4.57, max = 4.7)
```


```{r}
predict(model_df_full, type = "response")
```
